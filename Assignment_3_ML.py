...
pca_elements = np.linspace(25, 26, 2, endpoint=True)
...
kurt = []
for i in range(1,40):
    X_transformed =FastICA(n_components=i).fit_transform(X) #random_state=5)
    kurt.append(max(kurtosis(X_transformed,fisher=True)))

plt.figure()
plt.plot(range(1,40),kurt,label='Max Kurtosis')
plt.axvline(np.argmax(kurt),color='red',alpha=0.7,linestyle='--',label='Optimal # = 8')
...
transformer = GaussianRandomProjection(eps=0.5,n_components = 2, random_state=0)
X_rp = transformer.fit_transform(X)
randMat = transformer.components_
X_rp_inv = X_rp.dot(randMat)


rp_components = np.linspace(2,100,99,endpoint=True)
cosine_similarity_iterator =[]
cosine_similarity = []
for l in rp_components:
    transformer = GaussianRandomProjection(eps=0.5,n_components = int(l), random_state=0)
    X_rp = transformer.fit_transform(X)
    randMat = transformer.components_
    X_rp_inv = X_rp.dot(randMat)
    for u in range(len(X.iloc[0,:].values)):
        first = X.iloc[:,u].values
        first = first.reshape(1,-1)
        second = X_rp_inv[:,u]
        second = second.reshape(1,-1)
        cosine_similarity_iterator.append(1 - sp.distance.cdist(
                first, second, 'cosine'))
    cosine_similarity.append(np.average(cosine_similarity_iterator))
    

plt.figure()
plt.plot(range(0,len(rp_components)),cosine_similarity)
plt.xlabel('Number of RP components before reconstruction')
plt.ylabel('Cosine Similarity')
plt.title('Cosine Similarity per Number of Random Projections')
plt.show()
...
rp_components = np.linspace(2,8,7,endpoint=True)
cosine_similarity_iterator =[]
cosine_similarity = []
for l in rp_components:
    transformer = GaussianRandomProjection(eps=0.5,n_components = int(l), random_state=5)
    X_rp = transformer.fit_transform(X1)
    randMat = transformer.components_
    X_rp_inv = X_rp.dot(randMat)
    for u in range(len(X1[0,:])):
        first = X1[:,u]
        first = first.reshape(1,-1)
        second = X_rp_inv[:,u]
        second = second.reshape(1,-1)
